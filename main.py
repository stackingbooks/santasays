...
os.system('ffmpeg -f concat -i /app/xd.txt -t ' + str(length) + ' -i ' + talk + ' -i ' + audio + ' -c copy -y ' + output)
os.system('ffmpeg -i ' + start + ' -i ' + output + ' -i ' + end + ' -filter_complex "[0:v] [0:a] [1:v] [1:a] [2:v] [2:a] concat=n=3:v=1:a=1 [v] [a]" -map "[v]" -map "[a]" -strict -2 -y ' + final)
